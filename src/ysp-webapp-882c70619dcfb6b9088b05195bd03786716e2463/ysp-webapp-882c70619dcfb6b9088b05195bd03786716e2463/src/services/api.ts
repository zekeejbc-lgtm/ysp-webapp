...
export const homepageAPI = {
  ...
  /**
   * Update homepage content (Admin/Auditor only)
   */
  updateContent: async (
    updates: {
      mission?: string;
    ...
    idCode: string
  ): Promise<{ success: boolean; message?: string; updatedKeys?: string[] }> => {
    // Invalidate homepage cache when updating content
    invalidateCache('homepage_content');
    return apiRequest('updateHomepageContent', { ...updates, idCode });
  },
